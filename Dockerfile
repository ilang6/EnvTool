...
# Install mermaid.cli
RUN export PUPPETEER_SKIP_CHROMIUM_DOWNLOAD=true && npm install -g @mermaid-js/mermaid-cli && \
    npm update -g
...
RUN wget https://github.com/VSCodium/vscodium/releases/download/1.90.2.24171/codium_1.90.2.24171_amd64.deb && \
    dpkg -i codium_1.90.2.24171_amd64.deb && \
    wget -c https://github.com/quarto-dev/quarto-cli/releases/download/v1.5.53/quarto-1.5.53-linux-amd64.deb && \
    dpkg -i quarto-1.5.53-linux-amd64.deb && \
    /opt/conda/bin/pip install -r requirements.txt && \
...
# Remove the 'lunar-cran40' repository if it exists
RUN if grep -q "lunar-cran40" /etc/apt/sources.list.d/*; then \
        sed -i '/lunar-cran40/d' /etc/apt/sources.list.d/*; \
    fi
